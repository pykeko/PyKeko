...
## Current state (as of pk-v0.2.18, 2026-06-05)

- Version: `0.2.18` in `package.json`, `CFBundleShortVersionString` derived from it
- Release: pk-v0.2.18 on Moorhen-PyKeko (canonical, asset: `PyKeko.dmg`, **151 MB** after the forge.config ignore-list slim-down — was 226 MB at v0.2.17). The wrapper repo carries a matching `pk-v0.2.18` tag and a mirror release with the same dmg. The org page's download badge and install link point at the Moorhen-PyKeko release (historical convention since v0.1). When shipping a new version: build the dmg in `~/PyKeko` via `npm run make`, create a release on **Moorhen-PyKeko first** with the dmg + `--latest`, then mirror to PyKeko. Update `RELEASE-HISTORY.md` (use `tools/release-sizes.sh v0.2.X` for the new row).
