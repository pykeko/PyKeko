...
## Current state (as of pk-v0.2.9, 2026-06-03)

- Version: `0.2.9` in `package.json`, `CFBundleShortVersionString` derived from it
- Release: [pk-v0.2.9](https://github.com/pykeko/Moorhen-PyKeko/releases/tag/pk-v0.2.9) on Moorhen-PyKeko (canonical, asset: `PyKeko.dmg`). The wrapper repo carries a matching `pk-v0.2.9` tag and a mirror release at [pykeko/PyKeko/releases/tag/pk-v0.2.9](https://github.com/pykeko/PyKeko/releases/tag/pk-v0.2.9) with the same dmg, but the org page's download badge and install link point at the Moorhen-PyKeko release (historical convention since v0.1). When shipping a new version: build the dmg in `~/PyKeko` via `npm run make`, create a release on **Moorhen-PyKeko first** with the dmg + `--latest`, then mirror to PyKeko.
