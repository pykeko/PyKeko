...
## Current state (as of pk-v0.2.45, 2026-06-17)

- Version: `0.2.45` in `package.json`, `CFBundleShortVersionString` derived from it
- Release: pk-v0.2.45 on Moorhen-PyKeko (canonical, asset: `PyKeko.dmg`, **144 MB**). The wrapper repo carries a matching `pk-v0.2.45` tag and a mirror release with the same dmg. The org page's download badge and install link point at the Moorhen-PyKeko release (historical convention since v0.1). When shipping a new version: build the dmg in `~/PyKeko` via `npm run make`, create a release on **Moorhen-PyKeko first** with the dmg + `--latest`, then mirror to PyKeko. Update `RELEASE-HISTORY.md` (use `tools/release-sizes.sh` for the new row).
...
- Per-version detail for **v0.2.20 through v0.2.44** lives in [`RELEASE-HISTORY.md`](RELEASE-HISTORY.md) (reverse-chrono table) and the GitHub Releases page; below this point CLAUDE.md retains the detailed v0.2.18 → v0.2 notes that predate that table.
